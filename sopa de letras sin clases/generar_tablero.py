...
def generar_lugar_aleatorio(N, palabra):
    # horizontal = True
    # vertical = False
...
def insertar_palabra(palabra, tablero, lugar):
...
def insertar_al_diccionario(diccionario, palabra, posicion):
...
def generar_tablero(N, palabras):
    ...
    while palabras_insertadas < len(palabras):
        ...
        lugar = generar_lugar_aleatorio(N, palabra_actual)
        print("LUGAR GENERADO", lugar)
        (nuevo_tablero, inserto_la_palabra_correctamente, posicion) = insertar_palabra(palabra_actual, tablero, lugar)
        if inserto_la_palabra_correctamente:
            print("INSERTO", palabras[palabras_insertadas], inserto_la_palabra_correctamente)
            diccionario = insertar_al_diccionario(diccionario, palabra_actual, posicion)
            tablero = nuevo_tablero
            palabras_insertadas = palabras_insertadas + 1

    completar_tablero(tablero)
